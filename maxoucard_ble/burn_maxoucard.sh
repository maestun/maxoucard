#!/bin/bash
# usage burn_maxoucard <BLE_NAME> <BLE_AP_PASS>
CONFIG_FILE="./maxoucard_config.h"
SOURCE_FILE="./maxoucard_ble.ino"
...
then
	...
	echo "Usage: ./burn_maxoucard <BLE_NAME> <BLE_AP_PASS> [--skipcompile]"
else
	...
	rm $CONFIG_FILE
	touch $CONFIG_FILE
	echo "#define BLE_NAME \""$1"\"" >> $CONFIG_FILE
	echo "#define BLE_PASS \""$2"\"" >> $CONFIG_FILE

	# compile code
	if [ $# == 2 ] || [ $3 != "--skipcompile" ] || [ ! -f $BINARY_FILE ]
	then
		echo "compiling code..."
		rm -rf $BUILD_PATH
		mkdir $BUILD_PATH
